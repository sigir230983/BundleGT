...
class LiGT(nn.Module):
    ...
    def init_emb(self):
        ...
        self.eval_bundles = torch.arange(self.conf["n_bundle"]).to(self.device).long().detach().view(1, -1)
        self.eval_users = torch.arange(self.conf["n_user"]).to(self.device).long().detach().view(1, -1)
    ...
    def forward(self):
        ...
        u_feature_cf, i_feature_cf = self.LGN_ui(u_feature, i_feature, layer=None)
        ...
        for layer in range(self.conf["n_trans_layer"]):
            ...
            b_feature_T = self.LiT_bi(i_feature, b_i2b, self.conf["bi_alpha"], self.eval_bundles, layer=layer).squeeze(0)
            b_feature = b_feature_T + b_u2b 
            
            u_feature_T = self.LiT_ub(b_feature, u_b2u, self.conf["ub_alpha"], self.eval_users, layer=layer).squeeze(0)
